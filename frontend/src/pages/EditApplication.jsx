...
const EditApplication = () => {
    ...
    const fetchApp = async () => {
      try {
        ...
        if (app.applied_date) {
          ...
          setAppliedDate(appDate.toISOString().split('T')[0]);
        }
        if (app.deadline) {
          const dlDate = new Date(app.deadline);
          // format to YYYY-MM-DDTHH:MM
          const offset = dlDate.getTimezoneOffset();
          const localDlDate = new Date(dlDate.getTime() - offset * 60 * 1000);
          setDeadline(localDlDate.toISOString().slice(0, 16));
        }
      } catch (err) {
        console.error(err);
        setError('Failed to retrieve application details. Check if ID is valid.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const msg = err.response?.data?.message || 'Failed to update application. Check inputs.';
      setError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return <LoadingSpinner message="Fetching job data..." />;
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Modify tracking parameters for {companyName}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel rounded-3xl p-6 md:p-8 space-y-6 border border-brand-border/60 h-fit">
          ...
          >
            {submitting ? 'Updating record...' : 'Update Application'}
            <Save size={16} />
          </button>
        </div>
      </form>
