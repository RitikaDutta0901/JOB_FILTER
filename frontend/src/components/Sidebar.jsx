...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const links = [
    ...
    // If we build stats inside the Dashboard page or a separate page, we can route to /analytics or keep it in the same routes
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col justify-between py-6 px-4">
          ...
          {links.map((link) => {
            ...
            return (
              <NavLink
                ...
                `}
              >
                <Icon size={18} className={isActive ? 'text-brand-primary' : 'text-gray-400 group-hover:text-gray-300'} />
                {link.name}
              </NavLink>
            );
          })}
      ...
      <div className="border-t border-brand-border/60 pt-4 px-2">
        <button
          onClick={logout}
