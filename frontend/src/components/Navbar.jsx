...
import { LogOut, Menu, Bell, Briefcase } from 'lucide-react';
...
const Navbar = ({ onToggleSidebar }) => {
  ...
  return (
    <header className="sticky top-0 z-30 h-16 w-full border-b border-brand-border bg-brand-dark/80 backdrop-blur-md px-6 flex items-center justify-between">
      <div className="flex items-center gap-4">
        ...
        <Link to="/" className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-brand-primary to-brand-accent text-white shadow-lg shadow-indigo-500/25">
            <Briefcase size={18} className="stroke-[2.5]" aria-hidden="true" />
          </div>
      ...
      <div className="flex items-center gap-4">
        {/* Notifications Mock */}
        <button className="relative p-2 rounded-xl text-gray-400 hover:text-white hover:bg-brand-border/60 transition-colors" aria-label="Notifications">
          <Bell size={18} aria-hidden="true" />
          <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-brand-secondary" aria-hidden="true"></span>
        </button>
          ...
          {dropdownOpen && (
            <>
              <div
                ...
                aria-hidden="true"
              ></div>
              <div className="absolute right-0 mt-2 w-48 rounded-xl border border-brand-border bg-brand-card py-1.5 shadow-2xl z-20 animate-fade-in" role="menu">
                ...
                <div className="py-1">
                  <Link
                    to="/profile"
                    onClick={() => setDropdownOpen(false)}
                    className="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-brand-border/40 transition-colors"
                    role="menuitem"
                  >
                    Profile
                  </Link>
